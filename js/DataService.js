class DataService {
    ...
    // =====================================================
    // PLAYBOOK CRUD METHODS (mirroring signals pattern)
    // =====================================================

    static async loadPlaybooks() {
        try {
            console.log('Calling domo.get() for playbook data...');
            const response = await domo.get('/data/v1/playbook'); // Playbook endpoint

            if (response && response.length > 0) {
                console.log(`Successfully loaded ${response.length} playbook records from Domo API`);

                // Parse the Domo response data into playbooks format
                this.playbooks = this.parsePlaybookDomoResponse(response);
                console.log(`Processed ${this.playbooks.length} playbooks from Domo data`);

                return [...this.playbooks]; // Return copy of array
            } else {
                console.warn('No playbook data returned from Domo API, falling back to CSV data');
                this.playbooks = await this.loadPlaybookCSV();
                return [...this.playbooks];
            }
        } catch (error) {
            console.error('Failed to load playbooks from Domo API:', error.message || error);
            console.warn('Attempting to call domo.get() for playbooks failed - falling back to CSV data');

            // Fallback to CSV data when API call fails
            this.playbooks = await this.loadPlaybookCSV();
            console.log(`Successfully loaded ${this.playbooks.length} playbooks from CSV fallback`);
            return [...this.playbooks]; // Return copy of array
        }
    }
    ...
    static async getPlaybookById(playId) {
        if (this.playbooks.length === 0) {
            await this.loadPlaybooks();
        }
        const playbook = this.playbooks.find(p => p.play_id === playId || p.id === playId);
        return playbook ? { success: true, playbook } : { success: false, error: 'Playbook not found' };
    }

    static async createPlaybook(playbookData) {
        const newPlaybook = {
            id: `playbook-${Date.now()}`,
            play_id: playbookData.play_id || `PLAY-${Date.now()}`,
            created_date: new Date().toISOString().split('T')[0],
            ...playbookData
        };
    ...
    static async updatePlaybook(playId, updateData) {
        const playbookIndex = this.playbooks.findIndex(p => p.play_id === playId || p.id === playId);
        if (playbookIndex !== -1) {
            this.playbooks[playbookIndex] = { ...this.playbooks[playbookIndex], ...updateData };
            return { success: true, playbook: this.playbooks[playbookIndex] };
        }
        return { success: false, error: 'Playbook not found' };
    }

    static async deletePlaybook(playId) {
        const playbookIndex = this.playbooks.findIndex(p => p.play_id === playId || p.id === playId);
        if (playbookIndex !== -1) {
            const deletedPlaybook = this.playbooks.splice(playbookIndex, 1)[0];
            return { success: true, deletedPlaybook };
        }
        return { success: false, error: 'Playbook not found' };
    }

    // Playbook CSV loading method (similar to MasterDataLoader for signals)
    static async loadPlaybookCSV() {
        try {
            console.log('Loading playbook CSV data...');
            
            // Fetch the playbook CSV file with cache-busting
            const cacheBuster = `?v=${Date.now()}`;
            const response = await fetch(`./playbook-fallback.csv${cacheBuster}`);
            
            if (!response.ok) {
                throw new Error(`Failed to load playbook CSV: ${response.status}`);
            }
            
            const csvText = await response.text();
            console.log('Playbook CSV loaded successfully, length:', csvText.length);
            
            // Parse the CSV data
            const parsedData = this.parsePlaybookCSV(csvText);
            console.log(`Parsed ${parsedData.length} playbook records from CSV`);
            
            return parsedData;
        } catch (error) {
            console.error('Error loading playbook CSV:', error);
            // Return empty array if CSV loading fails
            return [];
        }
    }

    // Parse playbook CSV text into structured objects
    static parsePlaybookCSV(csvText) {
        const lines = csvText.split('\n').filter(line => line.trim());
        
        if (lines.length === 0) {
            console.warn('No data in playbook CSV');
            return [];
        }
        
        // Get header row and convert to object keys
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            if (values.length !== headers.length) {
                console.warn(`Playbook CSV row ${i + 1} has mismatched columns, skipping`);
                continue;
            }
            ...
            // Convert to playbook object using parsePlaybookDomoResponse
            const playbookData = this.parsePlaybookDomoResponse([rowData]);
    ...
    // Parse Domo response for playbook data (similar to parseDomoResponse for signals)
    static parsePlaybookDomoResponse(domoData) {
        console.log('Parsing playbook Domo response data...');

        const playbooks = [];

        domoData.forEach((row, index) => {
            const playId = row['PLAY_ID'] || row['Play ID'] || row['play_id'] || `PLAY-${index + 1}`;

            // Handle multiple possible column name variations
            const playName = row['Play Name'] || row['PLAY_NAME'] || row['play_name'] || 'Unknown Play';
            const description = row['Description'] || row['DESCRIPTION'] || row['description'] || 'No description available';
            const playType = row['Play Type'] || row['PLAY_TYPE'] || row['play_type'] || 'General';
            const pyramidLevel = row['Pyramid Level'] || row['PYRAMID_LEVEL'] || row['pyramid_level'] || '';
            const lifecycleStage = row['Customer Lifecycle Stage'] || row['CUSTOMER_LIFECYCLE_STAGE'] || row['customer_lifecycle_stage'] || '';
            const freePaid = row['Free v. Paid'] || row['FREE_V_PAID'] || row['free_v_paid'] || '';
            const tier = row['Tier (with CS Distintions)'] || row['TIER'] || row['tier'] || '';
            const receivingPersona = row['Receiving Persona'] || row['RECEIVING_PERSONA'] || row['receiving_persona'] || '';
            const initiatingRole = row['Initiating Role'] || row['INITIATING_ROLE'] || row['initiating_role'] || '';
            const executingRole = row['Executing Role'] || row['EXECUTING_ROLE'] || row['executing_role'] || '';
            const status = row['Status'] || row['STATUS'] || row['status'] || 'Active';
            const documentLocation = row['Current Document Location'] || row['CURRENT_DOCUMENT_LOCATION'] || row['current_document_location'] || '';
            ...
            const playbook = {
                ...
                free_v_paid: freePaid,
                tier: tier,
                receiving_persona: receivingPersona,
                initiating_role: initiatingRole,
                executing_role: executingRole,
                status: status,
        ...
        console.log(`Parsed ${playbooks.length} playbook records`);
        return playbooks;
    }
